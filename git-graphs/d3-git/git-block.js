...
export class Block {
  ...
  drawLinks() {
    ...
    const lnks = (this.origins || []).map((origin) => {
      ...
      const xTarget = ["top", "bottom"].includes(this.position)
        ? this.width / 2
        : 0;
    ...
    this.links = (this.block || this.container)
      .selectAll(`.${selector}`)
      .data(lnks)
      .enter()
      .append("path")
...
export class MergeCommit extends Block {
  constructor({ label, source, target }) {
    ...
    this.x =
      source.x > target.x ? source.branch.xNext() : target.branch.xNext();
  }
}
...
export class HeadTag extends Block {
  ...
  refresh(origin, tagPosition) {
    ...
    this.y =
      this.position === "top"
        ? this.branch.yPrevious()
        : this.branch.yNext({ xAxis: this.x });
    return this;
  }
}
...
export class BranchTag extends Block {
   ...
   */
  refresh({ origin, tagPosition, forceY }) {
    super.refresh(origin);
    if (origin && !tagPosition) {
      this.position = this.branch.tagPosition = origin.branch.tagPosition;
    } else if (tagPosition) {
    ...
    this.x = this.origins[0].x;
    if (forceY) {
      this.y = this.branch.yNext({ forcedY: this.origins[0].y });
    } else {
      this.y =
        this.position === "top"
          ? (origin.branch || this.branch).yPrevious(this)
          : this.branch.yNext({});
    }
    return this;
  }
