...
export class GitGraph {
  constructor({
    ...
    width = 1024,
    height = 500,
    viewBox = null, // SVG container dimensions
  }) {
    ...
    this.y = legend
      ? comments
        ? 2 * TEXT_HEIGHT + PADDING
        : this.blockHeight + this.yGap
      : 0;
  ...
  addBranch(branch) {
    ...
    this.detachCommits({
      ...
      commits: branch.detachedCommits,
    });
  ...
  processActions(actions) {
    actions.forEach((action) => {
      ...
      if (typeof this[method] !== "function") {
        throw new Error(
          `Unknown method ${method} for ${this.constructor.name}`
        );
      }
      this[method](action);
    });
  ...
  attachCommits({ branch, commits = [] }) {
    ...
    commits.forEach((label) => {
      ...
      if (commit) {
        commit.attach();
        // If head is set to the current commit, then refresh it
        if (source.branchTag && source.branchTag.origins[0] === commit) {
          source.branchTag.refresh({ forceY: true }).move();
          if (source.headTag) {
            source.headTag.refresh().move();
          }
        }
        // Manage detached head special case (only head is attached to commit)
        // If head is set to the current commit, then refresh it
        if (source.headTag && source.headTag.origins[0] === commit) {
          source.headTag.refresh().move();
        }
      }
    });
  ...
  detachCommits({ branch, commits = [] }) {
    ...
    commits.forEach((label) => {
      ...
      if (commit) {
        ...
        if (!firstCommit) {
          firstCommit = commit;
          source.y = source.yNext({});
          source.x = source.xNext(commit.origins[0]);
        }
      }
    });
  }
  ...
  merge({ label, source, target }) {
    [source, target] = [source, target].map(({ branch, commit }) =>
      this.findBranch(branch).findCommit(commit)
    );
   ...
   * param   {String} options.forceY Do we have to force Y position from targeted commit (default)
